...
function Schedule(props) {
  ...
  const getShiftStart = (shiftStart) => {
    ...
    let displayedString = "";
    date.getHours() < 10
      ? (displayedString += "0" + date.getHours())
      : (displayedString += date.getHours());
    displayedString += ":";
    date.getMinutes() < 10
      ? (displayedString += "0" + date.getMinutes())
      : (displayedString += date.getMinutes());
                ...
                {schedule.map((entry) => (
                  <td key={`${employee.id}+${entry.date}`}>
                    {entry.shifts.map((shift) =>
                      shift.scheduledEmployees.some(
                        (worker) => worker.name === employee.name
                      )
                        ? getShiftStart(shift.shiftStart)
                        : "Off"
                    )}
                  </td>
                ))}
